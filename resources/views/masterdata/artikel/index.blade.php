                ...
                <div class="mb-5 mt-4">
                    <div class="flex flex-wrap justify-between">
                                ...
                                <div class="flex items-center w-full mt-2 mb-2">
                                    ...
                                    <x-inputs.select name="paginate" id="paginate" class="form-select" style="width: 75px" onchange="window.location.href = this.value">
                                        @foreach([10, 25, 50, 100] as $value)
                                            <option value="{{ route('artikels.index', ['paginate' => $value, 'search' => $search]) }}" {{ $artikels->perPage() == $value ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                    </x-inputs.select>
                        ...
                        <div class="md:w-1/2 text-right">
                            <a href="{{ route('artikels.export_pdf') }}" class="button" style="background-color: rgb(129, 129, 129); color: white; transition: background-color 0.3s, color 0.3s;" onmouseover="this.style.backgroundColor='rgb(120, 120, 120)'; this.style.color='white';" onmouseout="this.style.backgroundColor='rgb(129, 129, 129)'; this.style.color='white';">
                                <i class="mr-1 icon ion-md-download"></i>
                                Pdf
                            </a>

                            <a href="{{ route('artikels.export_excel') }}" class="button" style="background-color: rgb(83, 138, 0); color: white; transition: background-color 0.3s, color 0.3s;" onmouseover="this.style.backgroundColor='rgb(72, 121, 0)'; this.style.color='white';" onmouseout="this.style.backgroundColor='rgb(83, 138, 0)'; this.style.color='white';">
                                <i class="mr-1 icon ion-md-download"></i>
                                Excel
                            </a>
                            @can('create', App\Models\Artikel::class)
                                <a href="{{ route('artikels.create') }}" class="button" style="background-color: #800000; color: white; transition: background-color 0.3s, color 0.3s;" onmouseover="this.style.backgroundColor='#700000'; this.style.color='white';" onmouseout="this.style.backgroundColor='#800000'; this.style.color='white';">
                            ...
                            <tr>
                                ...
                                @foreach($columns as $field => $label)
                                    <th class="px-4 py-3 text-left">
                                        <a href="{{ route('artikels.index', array_merge(request()->query(), ['sort_by' => $field, 'sort_direction' => ($sortBy === $field && $sortDirection === 'asc') ? 'desc' : 'asc'])) }}">
                                            {{ $label }}
                                            @if($sortBy === $field)
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center" style="width: 134px;">
                                    <div role="group" aria-label="Row Actions" class="relative inline-flex align-middle">
                                        @can('update', $artikel)
                                            <a href="{{ route('artikels.edit', $artikel) }}" class="mr-1">
                                        ...
                                        @endcan 
                                        @can('view', $artikel)
                                            <a href="{{ route('artikels.show', $artikel) }}" class="mr-1">
                                                <button type="button" class="button">
                                        ...
                                        @endcan 
                                        @can('delete', $artikel)
                                            <form id="deleteForm-{{ $artikel->id }}" action="{{ route('artikels.destroy', $artikel->slug) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <div role="group" aria-label="Row Actions" class="relative inline-flex align-middle">
                                                    <button type="button" class="button" onclick="confirmDelete('{{ $artikel->id }}')">

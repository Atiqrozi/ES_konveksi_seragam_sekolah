<x-app-layout>
    ...
    <div class="py-12 min-h-screen">
        <style>
            /* Mobile-specific layout fixes for posisi lowongan list (only affects <=768px) */
                    ...
                    <div class="flex flex-wrap justify-between">
                                ...
                                <div class="flex items-center w-full mt-2 mb-2">
                                    ...
                                    <x-inputs.select name="paginate" id="paginate" class="form-select" style="width: 75px" onchange="window.location.href = this.value">
                                        @foreach([10, 25, 50, 100] as $value)
                                            <option value="{{ route('posisi_lowongan.index', ['paginate' => $value, 'search' => $search]) }}" {{ $posisi_lowongans->perPage() == $value ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                    </x-inputs.select>
                        ...
                        <div class="md:w-1/2 text-right">
                            ...
                            @can('create', App\Models\PosisiLowongan::class)
                                <a href="{{ route('posisi_lowongan.create') }}" class="button" style="background-color: #800000; color: white; transition: background-color 0.3s, color 0.3s;" onmouseover="this.style.backgroundColor='#700000'; this.style.color='white';" onmouseout="this.style.backgroundColor='#800000'; this.style.color='white';">
                ...
                <div class="block w-full overflow-auto scrolling-touch">
                    <table class="w-full max-w-full mb-4 bg-transparent">
                            ...
                            <tr >
                                ...
                                @foreach($columns as $field => $label)
                                    <th class="px-4 py-3 text-left">
                                        <a href="{{ route('posisi_lowongan.index', array_merge(request()->query(), ['sort_by' => $field, 'sort_direction' => ($sortBy === $field && $sortDirection === 'asc') ? 'desc' : 'asc'])) }}">
                                            {{ $label }}
                                            @if($sortBy === $field)
                        ...
                        <tbody class="text-gray-600">
                            @forelse($posisi_lowongans as $key => $posisi_lowongan)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-left" style="max-width: 400px" data-label="No">
                                    {{ $posisi_lowongans->firstItem() + $key }}
                                </td>
                                <td class="px-4 py-3 text-left" style="max-width: 400px" data-label="Nama Posisi">
                                ...
                                <td class="px-4 py-3 text-center" style="width: 134px;" data-label="Action">
                                    <div role="group" aria-label="Row Actions" class=" relative inline-flex align-middle">
                                        ...
                                        @endcan 
                                        @can('view', $posisi_lowongan)
                                            <a href="{{ route('posisi_lowongan.show', $posisi_lowongan) }}" class="mr-1">
                                                <button type="button" class="button">
                                                    <i class="icon ion-md-eye"></i>
                                                </button>
                                            </a>
                                        @endcan 
                                        @can('delete', $posisi_lowongan)
                                            <form id="deleteForm" action="{{ route('posisi_lowongan.destroy', $posisi_lowongan->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <div role="group" aria-label="Row Actions" class=" relative inline-flex align-middle">
                                                    <button type="button" class="button" onclick="confirmDelete('{{ $posisi_lowongan->id }}')">
    ...
    <script>
        function confirmDelete(posisi_lowonganId) {
            Swal.fire({
                title: 'Apakah Anda Yakin?',
                text: 'Konfirmasi hapus posisi lowongan',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('deleteForm').action = '{{ route('posisi_lowongan.destroy', '') }}/' + posisi_lowonganId;

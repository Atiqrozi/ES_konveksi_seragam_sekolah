<x-app-layout>
    ...
    <div class="py-12 min-h-screen">
        <style>
            /* Mobile-specific layout fixes for pegawai list (only affects <=768px) */
                            ...
                            <tr >
                                ...
                                @foreach($columns as $field => $label)
                                    <th class="px-4 py-3 text-left">
                                        <a href="{{ route('pegawai.index', array_merge(request()->query(), ['sort_by' => $field, 'sort_direction' => ($sortBy === $field && $sortDirection === 'asc') ? 'desc' : 'asc'])) }}">
                                            {{ $label }}
                                            @if($sortBy === $field)

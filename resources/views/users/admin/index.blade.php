                                ...
                                <div class="flex items-center w-full mt-2 mb-2">
                                    ...
                                    <x-inputs.select name="paginate" id="paginate" class="form-select" style="width: 75px" onchange="window.location.href = this.value">
                                        @foreach([10, 25, 50, 100] as $value)
                                            <option value="{{ route('admin.index', ['paginate' => $value, 'search' => $search, 'sort_by' => $sortBy, 'sort_direction' => $sortDirection]) }}" {{ $admins->perPage() == $value ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                    </x-inputs.select>
                            ...
                            <tr>
                                ...
                                @foreach($columns as $field => $label)
                                    <th class="px-4 py-3 text-left">
                                        <a href="{{ route('admin.index', array_merge(request()->query(), ['sort_by' => $field, 'sort_direction' => ($sortBy === $field && $sortDirection === 'asc') ? 'desc' : 'asc'])) }}">
                                            {{ $label }}
                                            @if($sortBy === $field)
                        ...
                        <tbody class="text-gray-600">
                            @forelse($admins as $key => $admin)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 text-left">{{ $admins->firstItem() + $key }}</td>
                                    <td class="px-4 py-3 text-left">{{ $admin->nama ?? '-' }}</td>
                                    <td class="px-4 py-3 text-left">{{ $admin->email ?? '-' }}</td>
                                    <td class="px-4 py-3 text-left">{{ optional($admin->tanggal_lahir)->format('Y-m-d') ?? '-' }}</td>
                                    <td class="px-4 py-3 text-left">{{ $admin->updated_at ?? '-' }}</td>

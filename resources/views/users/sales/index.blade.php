<x-app-layout>
    ...
    <div class="py-12 min-h-screen">
        <style>
            /* Mobile-specific layout fixes for sales list (only affects <=768px) */
                        ...
                        <thead style="color: #800000">
                            <tr >
                                ...
                                @foreach($columns as $field => $label)
                                    <th class="px-4 py-3 text-left" style="min-width: 90px;">
                                        <a href="{{ route('sales.index', array_merge(request()->query(), ['sort_by' => $field, 'sort_direction' => ($sortBy === $field && $sortDirection === 'asc') ? 'desc' : 'asc'])) }}">
                                            {{ $label }}
                                            @if($sortBy === $field)

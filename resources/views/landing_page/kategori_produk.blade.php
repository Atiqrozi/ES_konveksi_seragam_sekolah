...
<body>
    ...
    <section class="produk" id="produk">
        ...
        <p>Menggunakan bahas berkualitas untuk setiap helai jahitannya</p>
        <div class="container_product">
            <ul class="card-list">
            @forelse($kategoris as $kategori)
                <li class="card">
                    <a href="{{ route('kategori.produk', $kategori->id) }}">
                        <div class="card-image"
                            style="background-image: url({{ \Storage::url($kategori->foto) }});">
                            <h4>{{ $kategori->nama }}</h4>
                        </div>
                    </a>
                </li>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const totalPages = {{ $kategoris->lastPage() }}; // Total pages from Laravel
            const currentPage = {{ $kategoris->currentPage() }}; // Current page from Laravel
